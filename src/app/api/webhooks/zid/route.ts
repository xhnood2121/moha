...
import { verifyWebhookSignature } from "@/lib/integrations/zid";
import { normalizeZidOrder, payloadHash } from "@/lib/integrations/normalize";
...
export async function POST(req: NextRequest) {
  ...
  const sig =
    req.headers.get("x-zid-signature") ?? req.headers.get("x-signature");
  if (!verifyWebhookSignature(raw, sig)) {
    return NextResponse.json({ error: "invalid_signature" }, { status: 401 });
  }

  let body: { event?: string; store?: { id?: string | number }; order?: unknown };
  try {
    body = JSON.parse(raw);
  } catch {
  ...
  const externalMerchantId = body.store?.id?.toString();
  if (!externalMerchantId) {
    return NextResponse.json({ error: "missing_merchant" }, { status: 400 });
  }

  const merchant = await prisma.merchant.findUnique({
    where: { platform_externalId: { platform: "ZID", externalId: externalMerchantId } },
  });
  ...
  if (eventType.startsWith("order.")) {
    const norm = normalizeZidOrder(eventType, body);
